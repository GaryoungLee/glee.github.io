...
Newell's model has been extended by applying the bounded acceleration model to proudce **realistic oscillations** and **hysterisis**. Stochastic desired acceleration has taken into account to incorporate human error with the oscillation. The stochastic vehicle displacement, $$\xi_{i}(\tau)$$, replaces the deterministic term of Newell's original model. 
